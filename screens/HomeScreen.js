  ...
  return (
    <View style={ringStyles.wrap}>
      <View style={ringStyles.svgWrap}>
        ...
        <Text style={[ringStyles.num, { color }]}>{passed}/{total}</Text>
      </View>
...
const ringStyles = StyleSheet.create({
  ...
  svgWrap: { width: 56, height: 56, alignItems: "center", justifyContent: "center" },
  track: { position: "absolute", width: 50, height: 50, borderRadius: 25, borderWidth: 3 },
  fill:  { position: "absolute", width: 50, height: 50, borderRadius: 25, borderWidth: 3 },
...
export default function HomeScreen({ navigation }) {
  ...
  const [loading, setLoading] = useState(true);           // ← from auth version
  ...
  useFocusEffect(
    useCallback(() => {
      (async () => {
        try {
          setLoading(true);
          const user = await authService.getCurrentUser();  // ← from auth version
          if (user) {
            const progress = await getProgress(user.id);   // ← passes userId
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.header, { opacity: headerOpacity }]}>
        ...
        >
          <View style={styles.headerTop}>
            ...
            {/* Right side: Tree btn + Logout btn */}
            <View style={styles.headerActions}>
              ...
              {/* ← Logout button from auth version */}
              <TouchableOpacity
          ...
          <View style={styles.progressRow}>
            <View style={styles.bigProgress}>
              <View style={styles.bigProgressRing}>
                <View style={[styles.bigProgressArc, { borderColor: "#3b82f6" }]} />
      ...
      {/* Loading spinner ← from auth version */}
      {loading ? (
...
const styles = StyleSheet.create({
  ...
  progressRow:    { flexDirection: "row", alignItems: "center", gap: 16, marginBottom: 14 },
  ...
  statCard:       { width: "46%", backgroundColor: "#1e293b", borderRadius: 10, padding: 8, alignItems: "center", borderWidth: 1, borderColor: "#334155" },
