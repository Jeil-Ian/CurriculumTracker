...
const STATUS_CONFIG = {
  passed:    { color: "#34d399", bg: "rgba(6,78,59,0.5)",   border: "rgba(52,211,153,0.3)",  label: "Passed",    icon: "✓", gradient: ["#064e3b", "#0f172a"] },
  enrolled:  { color: "#60a5fa", bg: "rgba(30,58,95,0.5)",  border: "rgba(96,165,250,0.3)",  label: "Enrolled",  icon: "◉", gradient: ["#1e3a5f", "#0f172a"] },
  failed:    { color: "#f87171", bg: "rgba(76,29,29,0.5)",  border: "rgba(248,113,113,0.3)", label: "Failed",    icon: "✗", gradient: ["#4c1d1d", "#0f172a"] },
  available: { color: "#fbbf24", bg: "rgba(69,26,3,0.5)",   border: "rgba(251,191,36,0.3)",  label: "Available", icon: "◈", gradient: ["#451a03", "#0f172a"] },
  locked:    { color: "#475569", bg: "rgba(15,23,42,0.5)",  border: "rgba(71,85,105,0.2)",   label: "Locked",    icon: "◌", gradient: ["#1e293b", "#0f172a"] },
};
  ...
  return (
    ...
    >
      ...
      <View style={[styles.relatedBadge, { backgroundColor: `${cfg.color}22`, borderColor: `${cfg.color}44` }]}>
        <Text style={[styles.relatedBadgeText, { color: cfg.color }]}>{cfg.icon} {cfg.label}</Text>
      </View>
    </TouchableOpacity>
  );
}
          ...
          <View style={styles.heroTop}>
            <View style={[styles.yearBadge, { borderColor: cfg.border }]}>
              <Text style={[styles.yearBadgeText, { color: cfg.color }]}>
                Year {subject.year} · {subject.sem === 3 ? "Summer" : subject.sem === 1 ? "1st Sem" : "2nd Sem"}
              </Text>
            </View>
          ...
          {["passed", "enrolled", "failed"].map((s) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Text style={styles.actionIcon}>{c.icon}</Text>
                  <Text style={[styles.actionLabel, { color: isActive ? c.color : "#64748b" }]}>
                    {c.label}
                  </Text>
                </LinearGradient>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  warningChip:     { backgroundColor: "rgba(251,191,36,0.15)", borderRadius: 6, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: "rgba(251,191,36,0.3)" },
